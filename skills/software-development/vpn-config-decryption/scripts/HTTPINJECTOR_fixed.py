...
class EHIDecryptor:
    ...
    FIXED 2026-08-10: `execute()` referenced `cls.BYPASS_IVS` / `cls.STANDARD_IVS`,
    which live on EHIConstants, NOT on EHIDecryptor -> AttributeError on every call.
    Both references now read `EHIConstants.BYPASS_IVS` / `EHIConstants.STANDARD_IVS`.

    VERIFIED 2026-08-10 on a real user .ehi file (Turkcell TR): returns the full
    config JSON wrapped in the HABIBIxNULLPTRO header. The wrapper must be stripped
    with `res[res.find("{"):res.rfind("}")+1]` before handing to the user.
    """
