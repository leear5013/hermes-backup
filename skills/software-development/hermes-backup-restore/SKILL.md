...
## Related
- `hermes-backup` (user-owned in this profile) — the backup direction: two-layer `state.db` redaction (config secrets + pattern scan) so GitHub secret scanning doesn't reject the push; PAT embedded in the script URL by design (rotate if leaked).
