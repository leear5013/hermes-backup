...
- `templates/restore-instruction.md` — paste-ready message the user hands to ANOTHER Hermes instance ("he will have the same GitHub repo access"); fills in repo URL, covers the same-token caveat (same account ≠ same local token), redacted state.db holes, `hermes curator adopt` after merge, and verify-the-repo-first before promising.
...
- **Guard blocks a TOKENLESS script too if it contains `rm -rf` / `git reset --hard`** (misclassified as a gateway lifecycle command). Verified 2026-08-09: `bash backup-to-github.sh` exits 1 in <0.1s with zero output (killed before any echo) and even `bash -x` shows the generic "cannot restart or stop the gateway" block — the script had NO token, only the destructive git ops. Cron runs are unaffected (cron bypasses the agent-terminal guard). **Verified agent-side workaround — manual guard-friendly push:**
  1. `git clone --depth 1 <repo>` (fresh) or `git fetch origin && git merge --ff-only origin/main` (update) — NEVER `reset --hard`.
  2. Copy files with plain `cp` (no `rm -rf` of the destination). Gotcha: `cp -r src dest` when `dest/` exists nests as `dest/src/` — use `cp -r src/. dest/` (once staged 615 bogus `skills/skills/` files this way).
  3. Redact the repo's `state.db` copy with the two-layer script (config secrets + pattern scan `gh[pousr]_…`, `sk-…`, `xox…`, `AKIA…`, `\d{8,10}:[A-Za-z0-9_-]{30,}`).
  4. Commit with the identity flag (repo has no local identity): `git -c user.name="Hermes Backup" -c user.email="hermes-backup@users.noreply.github.com" commit -m "auto-backup: …"`, then `git push origin main` (tokenless via `~/.git-credentials`).
  5. Verify the REMOTE: `git fetch origin && git log origin/main --oneline -1` (new SHA present) AND scan the committed artifact `git show origin/main:state.db | grep -aoE 'ghp_[A-Za-z0-9]{20,}|sk-[A-Za-z0-9]{20,}|[0-9]{8,10}:[A-Za-z0-9_-]{30,}'` → empty = clean (633 markers in one verified run).
