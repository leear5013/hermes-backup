...
description: Use when user asks for Follow-up cadence tracker — flag overdue/urgent applications, generate tailored email/LinkedIn follow-up drafts, record sent follow-ups.
version: 1.0.0
...
# Career Ops Followup — Career-Ops for Hermes
...
> "No active applications to follow up on. Apply to some roles first with `career-ops skills` and come back when they're aging."
...
- Suggest the user run `the corresponding Hermes skill {company}` to find the right person first
...
> - Trying a different contact via `the corresponding Hermes skill`
...
**To:** {email or "No contact found — run `the corresponding Hermes skill` first"}
