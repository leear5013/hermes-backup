...
description: Use when user asks for Portal scanner — 4-level discovery (local parser, ATS APIs, browser, WebSearch), title/location filtering, deduplication, pipeline export.
version: 1.0.0
...
# Career Ops Scan — Career-Ops for Hermes
...
> **Nota (v1.6+):** El escáner por defecto (`scan.mjs` / `npm run scan`) es **zero-token** y usa fuentes estructuradas: parsers locales configurados por empresa y APIs públicas de Greenhouse, Ashby y Lever. Los niveles con Hermes browser tools/`web_search` descritos abajo son el flujo **agente** (ejecutado por Claude/Codex), no lo que hace `scan.mjs`. Si una empresa no tiene parser local ni API Greenhouse/Ashby/Lever, `scan.mjs` la ignorará; para esos casos, el agente debe completar manualmente el Nivel 1 (Hermes browser tools) o Nivel 3 (`web_search`).
>
> **Regla (v1.8+):** Si el parser local de una empresa termina con éxito en Nivel 0, el agente **no** debe repetir esa empresa en Hermes browser tools (Nivel 1) ni en API (Nivel 2). En Nivel 3, las queries generales siguen activas, pero se descartan resultados de empresas ya cubiertas por parser. Ver [Regla: local parser exitoso](#regla-local-parser-exitoso--no-repetir-scraping-caro).
...
El objetivo de `scan_method: local_parser` es **reducir tokens**: evitar que el LLM vuelva a scrapear la misma empresa con Hermes browser tools o APIs redundantes.
...
|-------|----------------------------------------|
| **1 — Hermes browser tools** | **Omitir** — no `browser_navigate` a su `careers_url` (método más caro en tokens) |
| **2 — API** | **Omitir** — no `web_extract` de su `api:` (ya cubierta por parser; `scan.mjs` tampoco usa API tras parser exitoso) |
| **3 — `web_search`** | Ejecutar queries **generales** (`site:`, títulos de rol); **descartar** cada hit cuya empresa normalizada coincida con `local_parser_ok` |
...
- No crear queries `search_queries` dedicadas a una empresa con parser local activo (p. ej. `site:jobs.ashbyhq.com/cohere "AI Engineer"`); usar el parser o, si falla, Hermes browser tools/API.
...
### Nivel 1 — Hermes browser tools directo (PRINCIPAL)

**Para cada empresa en `tracked_companies` que no esté en `local_parser_ok`:** Navegar a su `careers_url` con Hermes browser tools (`browser_navigate` + `browser_snapshot`), leer TODOS los job listings visibles, y extraer título + URL de cada uno. Este es el método más fiable porque:
...
Para empresas con API pública o feed estructurado **que no estén en `local_parser_ok`**, usar la respuesta JSON/XML como complemento rápido de Nivel 1. Es más rápido que Hermes browser tools y reduce errores de scraping visual.
...
2. Nivel 1: Hermes browser tools → `tracked_companies` con `careers_url`, **excepto** `local_parser_ok`
3. Nivel 2: API → `tracked_companies` con `api:`, **excepto** `local_parser_ok`
4. Nivel 3: `web_search` → todos los `search_queries` con `enabled: true`; descartar hits de empresas en `local_parser_ok`
...
4. **Nivel 1 — Hermes browser tools scan** (paralelo en batches de 3-5):
...
7.5. **Verificar liveness de resultados de `web_search` (Nivel 3)** — ANTES de añadir a pipeline:
   ...
   Los resultados de `web_search` pueden estar desactualizados (Google cachea resultados durante semanas o meses). Para evitar evaluar ofertas expiradas, verificar con Hermes browser tools cada URL nueva que provenga del Nivel 3. Los Niveles 1 y 2 son inherentemente en tiempo real y no requieren esta verificación.

   Para cada URL nueva de Nivel 3 (secuencial — NUNCA Hermes browser tools en paralelo):
...
→ Ejecuta the corresponding Hermes skill para evaluar las nuevas ofertas.
```
...
3. Navegar con Hermes browser tools para confirmar que funciona
