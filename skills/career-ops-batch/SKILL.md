...
description: Use when user asks for Mass batch processing — conductor mode (headed browser) or standalone script, parallel sub-agent workers, resumable state.
version: 1.0.0
...
# Career Ops Batch — Career-Ops for Hermes
...
Each worker is a sub-agent via `delegate_task` with a clean 200K token context. The conductor only orchestrates. See the **Sub-agent Batch Mode** table in `AGENTS.md` for the correct command per CLI.
...
4. **For each pending URL**:
   ...
   d. Execute via Bash:
      ...
      ```bash
      # Use your CLI's headless command (see AGENTS.md — Sub-agent Batch Mode)
...
Each worker receives `batch-prompt.md` as a system prompt. It is self-contained. Use your CLI's headless command — see the **Sub-agent Batch Mode** table in `AGENTS.md`.
